    ...
    <!--  ********************************************************************************************* -->
		...
		<xsl:choose>	
			<xsl:when test="($isHot = 'Y') or ($isHotVal = 'Y') or ($isWebVal = 'Y') or ($isAutoEnq = 'Y') or ($isCheckFileEnri = 'Y') or ($isCaseConv!='') or ($isFrequency = 'Y')">
				...
				<xsl:attribute name="onKeyUp">invokeHelp(event,'<xsl:value-of select="fn"/>','issueToken');</xsl:attribute>
		...
		<xsl:if test="$stripFrameToolbars = 'false'">
		   <xsl:attribute name="ondblclick">javascript:help('<xsl:value-of select="fn"/>','','','issueToken')</xsl:attribute>
        </xsl:if>
	</xsl:template>
