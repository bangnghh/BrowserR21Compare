...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="generalForm">
		...
		<form name="generalForm" method="POST" action="BrowserServlet">
            ...
            <input type="hidden" name="drilltarget" id="drilltarget" value=""></input> 
			...
			<xsl:choose>
				<xsl:when test="responseDetails/messages/msg!=''">
					<input type="hidden" name="errorMessageFromT24"
						id="errorMessageFromT24">
						<xsl:attribute name="value"> <xsl:value-of
							select="/responseDetails/messages/msg" /> </xsl:attribute>
					</input>
				</xsl:when>
			</xsl:choose>
