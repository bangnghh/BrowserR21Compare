...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:variable name="user">
     ...
     <xsl:value-of select="/t24help/header/token"/>
 	 </xsl:variable>
                                                        ...
                                                        <a>
                                                            <xsl:choose>
                                                                <xsl:when test="$showStatusInfo='false'">
                                                                    <xsl:attribute name="onclick">javascript:viewCommonHelpFields('issueToken')</xsl:attribute>
                                                                    <xsl:attribute name="href">javascript:void(0)</xsl:attribute>
                                                                </xsl:when>
                                                                <xsl:otherwise>
                                                                    <xsl:attribute name="href">javascript:viewCommonHelpFields('issueToken')</xsl:attribute>
                                                                </xsl:otherwise>
                                                            </xsl:choose>
                                ...
                                <tr>
                                    <td>
                                        <xsl:if test="(substring($imagesDir, 2, 1) = ':')">
                                            <!-- DOS full path name with a drive letter -->
                                            <img border="0">
                                                <xsl:attribute name="src">../servlet/RenderHelpImageServlet?filePath=<xsl:copy-of select="$imagesDir"/>/<xsl:value-of select="image"/>&amp;formToken=<xsl:value-of select="$Token"/></xsl:attribute>
                                            </img>
                                        </xsl:if>
                                        <xsl:if test="(substring($imagesDir, 2, 1) != ':')">
                                            <!-- Unix path name relative to the web server root directory -->
